...
namespace PBL3.BLL
{
    public class BLL_Sanh
    {
        ...
        public List<SANH> GetCbSanh(DateTime NgayTC, int Ca, int SLBan)
        {
            List<SANH> data = new List<SANH>();
            CSDL db = new CSDL();
            var l = db.BILLs.Where(p => (DateTime.Compare(NgayTC.Date, (DateTime)p.BookingDate) == 0 && p.Time == (Ca + 1))).Select(p => p.IDSanh).ToList();
        ...
        //
        public List<CBBItem> AddCBBsanh(DateTime ngaytc, int caTC, int sl, int idB)
        {

            List<CBBItem> list = new List<CBBItem>();
            foreach (SANH item in GetCbSanh(ngaytc, caTC, sl, idB))
            {
                list.Add(new CBBItem { Value = item.IDSanh, Text = item.NameSanh });
            }
            return list;
        ...
        //
        public List<SANH> GetCbSanh(DateTime NgayTC, int Ca, int SLBan, int idB)
        {
            List<SANH> data = new List<SANH>();
            CSDL db = new CSDL();
            var l = db.BILLs.Where(p => (DateTime.Compare(NgayTC.Date, (DateTime)p.BookingDate) == 0 && p.IDBILL != idB && p.Time == (Ca + 1))).Select(p => p.IDSanh).ToList();
            foreach (SANH i in GetListSanh(0))
            {
                bool check = true;
                foreach (int j in l)
        ...
        //
        public SANH[] SortSanh(SANH[] arr)
        {
            for (int i = 0; i < arr.Length - 1; ++i)
            {
                for (int j = i + 1; j < arr.Length; ++j)
                {
                    if (arr[i].PriceSanh > arr[j].PriceSanh)
                    {
                        SANH temp = arr[i];
                        arr[i] = arr[j];
                        arr[j] = temp;
