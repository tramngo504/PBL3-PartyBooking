...
namespace PBL3.BLL
{
    class BLL_KhachHang
    {
        ...
        public List<CUSTOMER> CheckKH(string sdt, string cmnd)
        {
            CSDL da = new CSDL();
            var l1 = da.CUSTOMERs.Where(p => p.SDT.Equals(sdt) || p.CMND.Equals(cmnd));
            return l1.ToList();
        }
        ...
        public List<ShowKH> GetListKH(DateTime DateIn, DateTime DateOut)
        {
            ...
            var l = from c in db.BILLs
                    where (DateTime.Compare((DateTime)c.PayDay, DateIn) >= 0 && DateTime.Compare((DateTime)c.PayDay, DateOut) <= 0 && c.STATUS.Equals("Đã hoàn thành"))
                    select new ShowKH
                    {
                        IDBILL = c.IDBILL,
