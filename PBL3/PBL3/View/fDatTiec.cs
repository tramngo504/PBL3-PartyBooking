...
namespace PBL3
{
    public partial class fDatTiec : Form
    {
        ...
        {
            if (txbSL.Text != "" && cbbChonSanh.Text != "" && txbDatCoc.Text != "" && txbTD.Text != 0.ToString() && txbGiaSanh.Text != "")
            {
        ...
        {
            if (txbSL.Text.Equals(""))
            {
                txbTD.Text = (1 * (decimal)a).ToString();
            }
            else
            {
        ...
        private void txbSL_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!txbSL.Text.Equals("") && !txbSL.Text.Equals("0"))
            {
                AddCBBsanh(dtpTimeIn.Value, cbbCaTC.SelectedIndex, Convert.ToInt32(txbSL.Text));
                txbTD.Text = (BLL_MENUDETAIL.Instance.getTongTienMenuBan(ID_bill) * Convert.ToInt32(txbSL.Text)).ToString();

            }
            else
            {
                txbTD.Text = BLL_MENUDETAIL.Instance.getTongTienMenuBan(ID_bill).ToString();
                txbGiaSanh.Text = 0.ToString();
            }
        }
        ...
        private void txbTD_TextChanged(object sender, EventArgs e)
        {
            SetTXBTT(txbTD.Text, txbGiaSanh.Text, txbGiatiec.Text);
        }
    }
