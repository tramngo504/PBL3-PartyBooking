...
namespace PBL3
{
    public partial class fKhachHang : Form
    {
        ...
        {
            if (txbTenKH.Text != "" && txbSDT.Text.Length == 10 && txbCMND.Text.Length == 9 && txbDiaChi.Text != "")
            {
                CUSTOMER kh = new CUSTOMER { NameKH = txbTenKH.Text, SDT = txbSDT.Text, CMND = txbCMND.Text, DIACHI = txbDiaChi.Text };
                if (BLL_KhachHang.INSTANCE.CheckKH(kh.SDT, kh.CMND).Count > 0)
                {
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            else { MessageBox.Show("Thông tin không hợp lệ"); }
        }
        private void Reset()
        {
            txbIDKH.Text = "";
